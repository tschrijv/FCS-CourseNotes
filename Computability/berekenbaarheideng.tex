...
membership problem for these languages: does a given string belong to
...
\vspace{0.5cm} More formally:
 \grijs{\begin{definitie} Turing Machine\\
{\rm
...
% \clearpage
 \grijs{\begin{definitie} To recognize\\
{\rm
...
Its complementary definition is
 ...
 \grijs{\begin{definitie} Turing recognizable language\\
{\rm
...
This distinction is captured in the following definitions:
 ...
 \grijs{\begin{definitie} To decide\\
{\rm
...
\end{definitie}}
 ...
 \grijs{\begin{definitie} Turing decidable language\\
{\rm
...
Other terminology for {\em decidable language} is {\em recursive
 ...
 \grijs{\begin{definitie} Co-recognizable/co-decidable\\
\textup{A language L is \textbf{co-recognizable/co-decidable} if $\overline{L}$
recognizable/decidable is.}
\end{definitie}}

 \grijs{\begin{stelling}\label{corecognizable}
...
\end{proof}
 ...
 \grijs{\begin{stelling}\label{isdec}
...
The word {\em recursive} triggers often the image of a function
...
\grijs{\begin{stelling} $A_{TM}$ is not decidable.
\end{stelling}}
\begin{proof}
...
accept, but no point at which it rejects.
...
link with recognizable talen is quite direct. We had to wait
...
\end{itemize}
 ...
 \grijs{\begin{stelling}\label{reglandec}
...
\grijs{\begin{stelling}\label{acfg}
...
\grijs{\begin{stelling}\label{escfg}
...
\grijs{\begin{stelling}\label{reduction1}
...
the machinery needed to decide them: 
 ...
 \grijs{\begin{definitie} Linear Bounded Automaton\\
{\rm
A Linear Bounded Automaton is a (non-deterministic) Turing Machine
that read and writes only on the part of the tape that contained the
initial input.  }
\end{definitie}}
...
most a constant amount of extra space, meaning that the decision kan
...
$Neg_P = \{M|P(M) = false\}$.
 ...
 \grijs{\begin{definitie} Non-trivial property\\
{\rm
...
\grijs{\begin{definitie} Language invariant property\\
{\rm
The property $P$ is {\em language invariant} if 

$~~~~~~~~~L_{M_1} = L_{M_2} \Longrightarrow P(M_1) = P(M_2)$

or in words: {\em machines recognizing the same language either all
  have $P$m or none has $P$}.  }
\end{definitie}}
...
% \clearpage
 ...
 \grijs{\begin{stelling} Stelling I by Rice\\
Let $P$ be a non-trivial, language invariant property of the Turing
...
Emile Post was one of the founding fathers of recursion theory, and he
made contributions to universal of computing mechanisms. The word {\em
  correspondence} is related to {\em similarity, agreement}, not to
sending mail by post. Emile Post developed the following game:

You have a finite collection of dominos, each with two strings
(instead of the usual numbers). Of each type, there is an unlimited
supply for this game. If you put some dominos next to each other, you
see an upper string and a lower string. The question is: is it
possible to lay out a finite number of dominos to that the upper and
lower string are the same?
...
\caption{Four given stones and a solution\label{pcp1}}
\end{figure}
...
\vspace{0.5cm}
Figure~\ref{pcp1} shows that you don't need to use all dominos and
...
corresponding sequence of dominos.

This simple game (1) undecidable and (2) powerful enough to mimic all
...
(Turing) computable function\\ {\rm A function $f$ is (Turing)
...
rejecting end state.
 ...
 \grijs{\begin{definitie}\label{manyone} Many-one Reduction of languages\\
{\rm
...
\end{stelling}}
 ...
 \grijs{\begin{gevolg}
if $L_1 \leq_m L_2$ andn $L_1$ is not recognizable, then $L_2$ is not
...
$A_{TM}$. The relevant definition is:
 \grijs{\begin{definitie}  Turing reducible\\
{\rm
...
$~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~h(y+1) = g(y,h(y))$
 ...
 \grijs{\begin{definitie} Primitive recursive function\\
{\rm
...
\grijs{\begin{definitie} Recursive functions \\
{\rm
