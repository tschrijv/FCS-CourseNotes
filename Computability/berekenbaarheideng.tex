...
\begin{equation*}
H_{TM} = \{\langle M,s \rangle| M~\textit{is a Turing Machine that stops on input}~s\}
\end{equation*}
...
\begin{equation*}
A_{TM} = \{\langle M,s \rangle| M~\textit{is a Turing Machine and}~s \in L_{M}\}
\end{equation*}
...
We now construct a Turing machine $C$, which we call the \emph{contradiction machine}, as
\begin{equation*}
C(\langle M \rangle) = \mathit{opposite}(B(\langle M,M \rangle))
\end{equation*}
with $M$ a Turing machine, and
\begin{eqnarray*}
...
\Leftrightarrow  & \mathit{opposite}(B(\langle C, C \rangle)) & = & \mathit{reject} \\
\Leftrightarrow  & B(\langle C, C \rangle) & = & \mathit{accept} \\
...
\item
Given input $\langle RE,s \rangle$, first transform the RE to an DFA
...
$f$ maps $\langle M,s \rangle$ to $\langle M_s,M_\phi \rangle$;
...
collapse in case it turns out that $NP = P$. Our computability
...
compose them with simple operators. That is what Kurt Goedel and
Jacques Herbrand did.

%-------------------------------------------------------------------------------
\subsection{The Primitive Recursive Functions}

The first attempt of Goedel and Herbrand is known as the \emph{primitive
...
\begin{equation*}
\mathit{Cn}[f,g_1,\ldots,g_m](x_1,\ldots,x_n) = f(g_1(x_1,\ldots,x_n),\ldots,g_m(x_1,\ldots,x_1))
\end{equation*}
...
The recursion opertor allows us to define addition of natural numbers, which satisfies
...
was the arugment used to prove that it is not primitive recursive.
...
Goedel had to enlarge his class of functions to accomodate the Ackermann
...
the minimazation operator introduces WHILE-loops. Specifically, the oprator
...
\begin{verbatim}
       ...
       return y;
\end{verbatim}
This imperative implementation you can end up in an infinite loop in two
...
operator. The reasone for that is that, while it is possible, it is rather
