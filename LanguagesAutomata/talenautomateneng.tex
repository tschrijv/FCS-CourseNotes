...
In order to \martonst{fix}{define? describe?} a language, one must give a description for every
...
such that \martonst{xs}{} $L_{NFA} = L_{DFA}$
...
F \marton{(the set of the final states)}. On the other hand, states 3 and 5 do not have this property: the
...
distinguishable; repeat until no more such pair \marton{exists}
...
\martonst{$\delta^*(p,w) = \delta(\delta^*(p,v),z)$}{$\delta^*(p,w) = \delta^*(\delta^*(p,v),z)$}
...
arguments \martonst{hard}{more concrete}, if not now, certainly later:
\begin{enumerate}
...
\martonst{analyse}{analysis} generators.
...
\marton{What does omega mean here?}
...
appears later in this text) and other interesting issues (\martonst{a.o.}{e.g.} the
...
and even better\martonst{L}{,} every string in its language has exactly one parse
...
Some textbooks do not \marton{include} the rule $S$ \rpijl $\epsilon$: such a grammar
...
\item $\Sigma$ is a fine input alphabet
...
If $d \in L$, it has a parse tree. By deleting the
...
described easily by the following construction: first \martonst{rename apart the
nonterminals of the two given CFGs}{make sure the two CFGs don't share any nonterminal names, rename them if necessary}. Let the start symbols of the two
...
$L_2 = \{a^nb^mc^m|n,m \geq 0\}$. Clearly, the $L_i$\marton{s} are context
free\footnote{Make a CFG for them!}. The intersection of the $L_i$\marton{s} is
...
we \martonst{know in the mean time}{have found out}. Therefore, these parser generators impose
...
A CSL can be parser by a non-deterministic linear bounded automaton
