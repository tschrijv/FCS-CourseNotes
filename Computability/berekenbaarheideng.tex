...
\marton{Maybe a clarification in the definition about L,R,S, as this is the first time we abbreviate}
...
parallel edges are treated together) - except for the \martonst{start and end}{accepting and rejecting}
...
well). So, a \martonst{part}{subset} of $\N$ is a language. Commonly, one reserves the
...
\marton{Why is the halting problem capitalized everywhere?}
...
accept\marton{ing state}, but no point at which it rejects.
...
or \martonst{all of}{} the regular language $\Sigma^*$. So, $B$ accepts $\langle
...
\martonst{language invariant}{semantic?}
...
made contributions to \martonst{universal}{??} of computing mechanisms. The word {\em
...
\subsection{\martonst{Turing}{?} a Turing Machine in a PCP game}
...
because a method to solve \martonst{L1}{L2} is (together with the reduction) powerful
enough to solve \martonst{L2}{L1}. We will make all this very precise. It is important
...
\marton{The placement of the footnote number here can be misinterpreted as a vector length}
...
analysis of algorithms, \martonst{a.o.}{e.g.} for the Union-Find algorithm.
