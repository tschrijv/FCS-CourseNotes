...
% \clearpage
Now is the time to describe an algorithm for constructing \martonst{an NFA from
an RE}{a RE from an NFA}.
...
by first using the symbol $a$ \martonst{an}{in} each NFA state in ${\cal Q}$ using
...
Not every \martonst{CFGL}{CFL} has a non-ambiguous CFG: such a CFL is named {\em
...
If $d \in L$ \martonst{d}{s}, it has a parse tree. By deleting the
...
pumping lemma to prove that L is not in CFL. But \marton{$\overline{L}$?} is
