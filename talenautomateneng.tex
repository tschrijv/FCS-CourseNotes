\chapter{Languages and Automata}\label{chap:languagesautomaten}
...
... The output could be a sequence of signals controlling some valves,
...
the best day of the week for investing time in FCW (the output domain
has 7 values), you could ask the following yes/no questions {\em is
monday the best day for investing time in FCW?}, {\em is tuesday the
best day for investing time in FCW?} ... So, it seems that of all
...
or even more exotic excess-3, ... These representations have something
...
In order to fix a language, one must give a description for every
...
\item 
Does such a formalism allow to derive (automatically) testers and
...
\begin{itemize}
\item[] Why should an computer scientist know all this?
\end{itemize}
...
Can you see from the examples a connection between the ease of
...
operations, often binary operations, but also unary operations are
common, and even opreations with larger arity. The set of languages
over the alphabet $\Sigma$ becomes an algebra by defining operations
like union, intersection, complement ... More concrete:
...
scientists need a different formalism: it should allow (more easily)
...
exceptions to rules, and exceptions to exceptions ... It makes sense
...
\section{Regular expressions en regular languages}
...
\item $(E_1)^*$ regular expression $E_1$ over $\Sigma$
\item $(E_1 | E_2)$ for regular expressions $E_1$ and $E_2$ over $\Sigma$
\end{itemize}
}
\end{definitie}}

In this way, the set set of regular expressions {\em RegExps} (over an
alphabet $\Sigma$) is defined inductively. It implies that something
that does not complies to the definition, is not a regular expression,
...
{\rm
\begin{itemize}
\item if E = a ($a \in \Sigma$) then $L_E = \{a\}$ (the langusge with
...
formulas below mae sense, and which are true.
...
\item the vertices have a name (in this case an number) - the vertices
  are named {\em states}
...
\item any time you get at a final state, you may say {\em this machine generates the current string}
\end{enumerate}
...
construct an NFA with a circuit - now you can get into an endless
...
We need often the set $\Sigma \cup \{\epsilon\}$: we abbreviate it by
$\Sigma_\epsilon$.
...
As you guessed by now: NFA is acronym if {\em Non-deterministic Finite
  Automaton}.


We define formally what it means for a string $s$ to be accepted by an
NFA.
...
For combination of a state in the NFA ad a symbol in
...
Given $NFA_1 = (Q_1,\Sigma,\delta_1,q_{s1},\{q_{f1}\})$ en 
%
...
\item $\delta$ is defined as
 ...
 $\delta(q_{fi}, x) = \emptyset ~~\forall x \in \Sigma$ en for i = 1,2
\end{itemize}
...
We have now all necessary ingredients to construct from a regular
expression RE an NFA such that $L_{RE} = L_{NFA}$. Since regular
expressions are defined inductively (see definition page
\pageref{defregexp}) we define for each case in the definition a
corresponding NFA. $NFA_{RE}$ denotes the NFA corresponding to a
...
\item there is exactly one edge between every other two state (in both
...
the substring has lenght zero; if the edge just contains $\phi$, you
...
% \clearpage
Now is the time to describe an algorithm for constructing an NFA from
an RE.
...
The GNFA has exactly two state: the start and the accepting
...
it is also accepted after elimination; (2) if a string is not accepted
...
that for some $n>0$, AX$^n$B ($n>0$) is a subsequence in the path. The
...
contain X. Let AB be a subsequence of the path: on the edged from A to
...
string or not, and one has the choice to which state to
move. Implementing this (on the basis of the transition table for
...
optimal. It would be more efficient if there are no $\epsilon$-edges,
and if for each symbol in the alphabet, there were only one edge with
...
\begin{itemize}
\item[{\bf Given:}] a NFA = $(Q_n,\Sigma,\delta_n,q_{sn},F_n)$

\item[{\bf To construct:}] a DFA = $(Q_d,\Sigma,\delta_d,q_{sd},F_d)$
such that xs$L_{NFA} = L_{DFA}$
...
by first using the symbol $a$ an each NFA state in ${\cal Q}$ using
...
that the DFA has less states than the NFA you start from?
\end{itemize}
...
\caption{ At the left a DFA with 2 {\em equivalent} states; thet were
collapsed at the right.\label{mini1}}
\end{figure}
...
F. On the other hand, states 3 and 5 do not have this property: the
...
We describe first informally an algorithm to compute sets of
...
fro $p$ and $q$ to distinguishable states; now mark $p$ and $q$ as
distinguishable; repeat until no more such pair
...
\begin{enumerate}
\item[{\bf Init:}]
Consider the graph $V$ with as vertices the states of the DFA; add an
...
$\underline{\Longleftarrow}$: if $p$ and $q$ distinguishable, then
...
\grijs{\begin{stelling}
If $DFA_1 = (Q_1,\Sigma,\delta_1,q_s,F_1)$ is a machine all of whose
states are reachable, and with every pair of states distinguishable,
then there is no equivalent DFA with strictly less states.
\end{stelling}
}
\begin{proof}
...
state. Assume that 
%
$DFA_2 = (Q_2,\Sigma,\delta_2,p_s,F_2)$ is a DFA with strictly less
...
arguments hard, if not now, certainly later:
\begin{enumerate}

\item
the number of DFAs (over a fixed alphabet) is countably infinite; the
number if languages is uncountably infinite ...
...
$(q_s=q_1,q_2,...,q_{n+1})$: its lengths is strictly larger that $d$,
...
string as $abc = xyz$ with $x = \epsilon$, $y = a$ en $z =
...
\label{flexlabel}
The description of a lexicon can be used to construct automatically a
{\em lexer} for Java, i.e. a program that takes a Java program as input,
and breaks it up in lexical units. You are at this moment actually
...
handling). jflex (\verb|http://www.jflex.de/|) is based on the same principle and generates Java code. flex en jflex are both lexical
analyse generators.
...
Wite a program (pick your language) that takes as input a regular
...
$of([star(a),b],[c])$ (but you are welcome to try another one).
\end{itemize}
...
meant for context free languages. BNF can be used also for regular
...
groups of 3 bits occurring in the i-th place, and write those groeps
in a sequence. We obtain so
...
Sometimes, one name a DFA a {\em one-way finite automaton}. The reason
is that one describe the DFA as a machine with an input tape on which
...
build a B\"{u}chi automaton for the language $(a|b)^*b^\omega$.
...
Stephen Kleene: you might know him from fixpoint theorems. He made also contributions to logic, recursion theory, and intuitionism.
...
appears later in this text) and other interesting issues (a.o. the
{\em Angels and Devils game}) that affect directly our understanding
of algorithmics.
...
write down formally the definition of L3. The question is always the
...
from L1 and L2 (also write down formally what merging means!)
...
\section{Context free languages and their grammar}\label{contextvrijelanguages}
...
whether a string complies to the grammar. Generation is done by
...
\item $s_0 = b$
\item $s_n = f$
\item $s_{i+1}$ is obtained from $s_i$ (voor $i < n$) by replacing in
 $s_i$ a nonterminal $X$ by the right hand side of a rule in which
...
set of string over $\Sigma$ that can be derived from $S$; or more
...
The string a+a*a seems to have more than one leftmost derivations and
...
and even betterL every string in its language has exactly one parse
tree for Arit2. We say: Arit2 is a non-ambiguous grammar.


Not every CFGL has a non-ambiguous CFG: such a CFL is named {\em
...
Some textbooks do not the rule $S$ \rpijl $\epsilon$: such a grammar
...
pumping lemma). On top of that, every derivations of a string of
...
Prologprogramma with in each clause body zero or two goals?
\end{itemize}
...
result, it is incapable to determine lots of interesting
...
\begin{itemize}
\item $Q$ is a finite kset of states
\item $\Sigma$ is a fine inputal phabet
...
$a^2bc^2$ and empty stack, the stack contains $mm\$$, and the string
...
$w_i$, i= 1..m ($w_i \in \Sigma_\epsilon$), and there exisat states
...
... All these are equivalent in terms of the set of languages that can
be determined by these machines. So, do not get to much hung up by the
...
It is only true of for this PDA you define acceptance as {\em arrive
...
\item the grammar rules: for each rule $X \rightarrow \gamma$ there is
  ...
  replace it by $\gamma$; $\gamma$ is sequence of terminals and
  nonterminals
\end{itemize}
...
We use the PDA to parse string $a+a*a$: Table~\ref{parsing1} shows the
stack and the string during during parsing. Note that when we push the
...
Is it possible that you get stuck? Try E*E instead of E$+$E at the 3th
...
give the impression that the grammar is ambiguous, but is not true: grammar
...
The construction of a PDA from a CFG is so easy: you need only 3
...
\end{itemize}
We mentioned this form earlier - now is a good time to proof that it
is not restrictive.
...
you in the PDA from state p with empty stack to state q with empty
...
\paragraph{Consequence II:} every regular language is context free. The reason is that an FSA is a also a PDA: it just ignores the stack.
...
\end{stelling}}
\begin{proof}
We use once more the pigeon hole principle, nnow on the parse tree for
strings that are long enough. Consider a CFG in Chomsky normal form
for L. Let $n$ be the number of nonterminals.
\end{proof}

If $d \in L$, it has a parse tree. By deleting from that tree the
leaves, we get a full binary tree - because of the Chomsky normal
...
$n$, so it has at ost $2^{n-1}$ leaves which correspond exactly with
...
described easily by the following construction: first rename apart the
nonterminals of the two given CFGs. Let the start symbols of the two
...
$L_2 = \{a^nb^mc^m|n,m \geq 0\}$. Clearly, the $L_i$ are context
free\footnote{Make a CFG for them!}. The intersection of the $L_i$ is
...
The converse it not true: there exist non-ambiguous non-deterministic
...
pumping lemma to prove that L is not in CFL. But is
...
we know in the mean time. Therefore, these parser generators impose
...
that if you even need a PDA, you remember that tools can do it for
you.


The 3-state PDA we constructed from a CFG gave us leftmost topdown
derivations: the parse tree is buildstarting from the start symbol
...
rule. Hier is a worked out example for the input $a+a*a$.
...
a grammar rule. Such a machine needs also an internal state (not shown
...
the lef hand side of a rule could have only one symbol, a nonterminal.
...
A CSL can be parser by a non-deterministic lineair bounded automaton
